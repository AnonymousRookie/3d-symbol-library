...
void CMy3DSymbolLibNewView::OnSize(UINT nType, int cx, int cy)
{
	...
	if(cy>0)
	{
		...
		float m_aspectRatio = (float)cx/(float)cy ; // 视口的横纵比例
...
void CMy3DSymbolLibNewView::InitTerrain()
{
	...
	for (int z = 0; z < MAP_W; z++)
		...
		{
			Vertex = z * MAP_W + x;
			g_terrain [Vertex][0] = float(x)*MAP_SCALE;					// 地域数组 3维，MAP_SCALE是边长
			g_terrain [Vertex][1] = (float)(g_imageData[Vertex*3] / 3);	// 地域数组 3维 灰度等高线生成高度图，在Modelobj初始化就赋值了
				
			
			g_terrain [Vertex][2] = -float(z)*MAP_SCALE;					// 地域数组 3维

			g_texcoord[Vertex][0] = (float) x;							// 索引数组2维，指示曲面贴图的平面坐标
	...
	if(winZ>=0 && winZ<1.0) 
	{
		...
		{ 			
			...
			if(m_bMouseMove3DModelPtNums == 1)			// 如果只选择了1个点
			{
				// JudgeRayIntersect(nearPoint, n_vector, resultP);
				PCordinate ppt = new Cordinate;  
				ppt->x=wx; ppt->y=wy; ppt->z=wz;			// 三维坐标
...
{ 
	...
	CVector3 pos(x,y,z);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0,0.0);glVertex3f((pos+(X+Z)*-h).x, (pos+(X+Z)*-h).y, (pos+(X+Z)*-h).z);	// 左下点
	glTexCoord2f(1.0,0.0);glVertex3f((pos+(X-Z)* h).x, (pos+(X-Z)* h).y, (pos+(X-Z)* h).z);	// 右下点
	glTexCoord2f(1.0,1.0);glVertex3f((pos+(X+Z)* h).x, (pos+(X+Z)* h).y, (pos+(X+Z)* h).z);	// 右上点
	glTexCoord2f(0.0,1.0);glVertex3f((pos+(Z-X)* h).x, (pos+(Z-X)* h).y, (pos+(Z-X)* h).z);	// 左上点
	glEnd();
	glDisable(GL_ALPHA);
	glDisable(GL_BLEND);
...
{ 
	...
	glTexCoord2f(1.0f, 0.0f); glVertex3f( h, 0, 0.0f);		// 右上点
	glTexCoord2f(0.0f, 0.0f); glVertex3f( 0, 0, 0.0f);		// 右上点
	glTexCoord2f(0.0f, 1.0f); glVertex3f( 0, h, 0.0f);		// 右下点
...
float CMy3DSymbolLibNewView::RayIntersect(
	...
	CVector3 max
		...
		if(rayStart.x<min.x){//如果射线的起点在左边界以左  
			...
			if(xt>rayDir.x){ return kNoIntersection; }//如果射线更偏左则无法相交  
			//这里是什么意思？？？？？？？？？？？？？？？？？？？？？？  
			//我想着是有什么几何意义吧，有哪位高人知道？？？？？？？？？？？  
			...
			if(xt<rayDir.x){ return kNoIntersection; }//如果射线更偏右则无法相交  
			xt /= rayDir.x;  
			inside = false;  
			xn = 1.0f;  
		}  
...
{
	char cc[256];
	strcpy(cc,terrainTex);				// 地面贴图
	LoadT8(cc,	texTerrain);	

	strcpy(cc,terrainContour);			// 等高地势图
...
//	if(file == NULL)
//	{
//		MessageBox("场景配置文件不存在!", "场景配置文件导入", MB_ICONINFORMATION + MB_OK);
//		return FALSE;
//	}
//	else
//	{ 
				...
				int colNum, int col_index_begin, int col_index_end, Line3 _line, vector<Point3> &_pv)
{
	...
	if(row_index_begin < row_index_end)
	{
		for(int r=row_index_begin+1; r<row_index_end; ++r)
		{
			...
			tmp_point._x = float(r)*MAP_SCALE;	// 平面横坐标
	...
	if(col_index_begin < col_index_end)
	{
		...
		{
			tmp_point._z = float(-c)*MAP_SCALE;	// 平面纵坐标
...
void CMy3DSymbolLibNewView::LoadAreaTexture(CString _areaTexture_str, UINT &texture_id)// 加载面符号纹理
{
